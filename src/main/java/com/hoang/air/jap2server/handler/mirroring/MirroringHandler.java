package com.hoang.air.jap2server.handler.mirroring;
...
public class MirroringHandler extends SimpleChannelInboundHandler<ByteBuf> {
    ...
    public MirroringHandler(AirPlay airPlay, AirplayStream dataConsumer) {
        this.airPlay = airPlay;
        this.video = dataConsumer;
    }
    ...
    private void processVideo(byte[] payload) {
        ...
        while (nalu_size < payload.length) {
            ...
            if (payload.length - nc_len > 4) {
                log.error("Decrypt processVideo error ! ");
                return;
            }
        }
        video.onVideo(payload);
    }
