package com.hoang.air.jap2lib.sec;

import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import java.util.Arrays;

import static com.hoang.air.jap2lib.sec.OmgHaxConst.*;
...
public class OmgHax {
    ...
    void decryptMessage(byte[] messageIn, byte[] decryptedMessage) {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            for (int j = 0; j < 9; j++) {
                ...
                buffer[0x0] = (byte) (message_table_index(base)[buffer[0x0] & 0xFF] ^ message_key[mode][base]);
            ...
            buffer[0x0] = table_s10[(buffer[0x0] & 0xFF)];
    ...
    private void permute_block_2(byte[] block, int round) {
        block[0] = permute_table_2(round * 16)[(block[0] & 0xff)];
